<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>登录密码:</label>
        <input type="password" placeholder="请输入你的密码" v-model="password" name="password" v-validate="{required:true,regex:/^[0-9A-z]{8,20}$/}" :class="{invalid:errors.has('password')}"/>
...
<script>
  export default {
    ...
    methods: {
      // 获取验证码
      async getCode(){
        // 简单判断--至少有数据（将phone解构出来，使用的时候就不用加this了）
        try {
          // 如果获取到验证码
          const {phone}=this
          phone&&await this.$store.dispatch('getCode',phone)
          // 将组件的code属性值变为仓库的验证码【验证码自己填写了】
      ...
      async userRegister(){
        ...
        // 如果表单验证成功，再向服务器发请求进行注册，只要有一个表单没有成功，则不会发请求
        if(success){
          try {
            // 如果成功----路由跳转
