...
export default {
  ...
  beforeMount() {
    ...
    // #endregion
    // 在发请求之前，把接口需要传递的参数进行整理（在给服务器发强求之前，把参数整理好，服务器就会返回查询的数据）
   Object.assign(this.searchParams,this.$route.query,this.$route.params)
  },
  mounted() {
    // 在发请求之前带给服务器参数【searchParams】参数发生变化有数值发生变化
    this.getData();
  },
  computed: {
    // mapGetters里面的写法：传递的是数组，因为getters计算是没有划分模块【home，search】的
  ...
  methods: {
    ...
    // 删除分类的名字---面包屑中
    removeCategoryName(){
      // 把带给服务器的参数置空，还需要向服务器发请求（）设置为undefined，属性值不会传给服务器
      // 将当前字段变为undefined，当前的字段不会带给服务器
      this.searchParams.categoryName=undefined
      this.searchParams.category1Id=undefined
      this.searchParams.category2Id=undefined
      this.searchParams.category3Id=undefined
      this.getData();
      // 更改地址栏：进行路由跳转（当路径中出现params时，不应该删除params，只删query参数）
    ...
    // 删除关键字信息---面包屑中
    removeKeyword(){
      // 1.将服务器带的参数searchParams中的keyword置空
      this.searchParams.keyword=undefined
      // 2.再次发请求
      this.getData();
      // 3.通知兄弟组件兄弟组件header组件清除关键字
      this.$bus.$emit('clear')
      // 进行路由的跳转
      if(this.$route.query){
  ...
  watch: {
    $route(){
      // 每一次请求前，应该把相应的1,2,3级分类的id置空，让它接收下一次相应的1,2,3id
      this.searchParams.category1Id=undefined
      this.searchParams.category2Id=undefined
      this.searchParams.category3Id=undefined
      // 每次发请求前都要重新整理服务器参数
