        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 
                  之前的路由跳转就是从A路由跳转到B路由，这里在加入购物车，进行路由跳转之前，
                    发请求，把购买的产品信息通过请求的形式通知服务器，服务器进行相应的存储
                 -->
...
export default {
  ...
  methods: {
    ...
    // 加入购物车的回调函数
    async addShopCart(){
      // 1.在点击和购物车按钮后，做的第一件事，将参数带给服务器（发请求），通话服务器加入购物车的产品是谁
      /* 
        ...
        上面这行代码实际上是调用addOrUpdateShopCart函数，这个方法加上async，返回的一定是Promise，要么成功|要么失败
      */
      ...
      try {
        ...
        // 3.进行路由跳转
        // 4.在路由跳转时还需要将产品的信息带给下一级的路由组件
        // 一些简单的数据skuNum，通过query形式给路由组件传递过去
        // 产品信息的数据【比较复杂：skuInfo】，通过回话存储（不持久化，会话结束，数据消失）
        // 本地存储|会话存储，一般存储字符串
